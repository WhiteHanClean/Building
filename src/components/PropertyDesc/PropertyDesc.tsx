import {
  ...
  paramsSecond,
  paramsThird,
} from "@/constans/PropertyDescData";
    ...
    <div className={s.property_desc}>
      <div className={s.property_desc_field}>
        <div className={s.porperty_desc_description}>
          <h3>Описание недвижимости</h3>
          <p>
            По потопе трое сыновей Ноя разделили землю – Сим, Xaм, Иaфeт. И
            достался восток Симу:Персия, Бактрия, даже и до Индии в долготу, а в
            ширину до Ринокорура, то есть от востока идо юга, и Сирия, и Мидия
            до реки Евфрат, Вавилон, Кордуна, ассирияне, Месопотамия,Аравия
            Старейшая, Елимаис, Инди, Аравия Сильная, Колия, Коммагена, вся
            Финикия. <span>Год постройки 2020</span>
            <br />
            <br />
            <span>
              Хаму же достался юг: Египет, Эфиопия, соседящая с Индией, и другая
              Эфиопия, из которойвытекает река эфиопская Красная, текущая на
              восток, Фивы, Ливия.
            </span>
          </p>
          <div className={s.property_desc_description_facilities}>
            {facilitiesTop.map((item, index) => (
              <div
                className={s.property_desc_description_facilitie}
                key={index}
              >
        ...
        <div className={s.property_desc_information}>
          <div className={s.property_desc_information_column}>
            {paramsSecond.map((item, index) => (
              <div
                className={s.property_desc_information_column_card}
                key={index}
              >
                <h3>{item.title}</h3>
                <p>{item.desc}</p>
